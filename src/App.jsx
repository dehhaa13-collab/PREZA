...
const slides = [
  ...
  { id: 6, type: 'title', title: 'З НАМИ ТИ', highlight: 'ОТРИМАЄШ РЕЗУЛЬТАТ, НАВІТЬ ЯКЩО...' },
  {
    id: 7, type: 'objections', items: [
      ...
      { text: 'СУМНІВАЄШСЯ В СОБІ', invert: true }, { text: 'ЩІЛЬНИЙ ГРАФІК ЗАПИСІВ', invert: false },
      { text: 'НІКОЛИ НЕ ВЕЛА БЛОГ', invert: true }, { text: 'ПРАЦЮЄШ У МАЛЕНЬКОМУ МІСТІ', invert: false }
    ]
  },
  ...
  },
  {
    id: 10, type: 'cost-comparison', title: 'СКІЛЬКИ ЦЕ КОШТУЄ,\nЯКЩО РОБИТИ САМОСТІЙНО', items: [
      ...
      { title: 'ЗАПУСК РЕКЛАМИ', cost: '100€ (5 000 ГРН)' }
    ...
    return (
      <motion.div initial="hidden" whileInView="show" viewport={viewConfig} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '40px 0' }}>
        ...
        <motion.div variants={itemAnim} className={isDanger ? "pill-danger" : "pill-primary"}>
          <span className="title-font clamp-h2-large text-3d" style={{ fontWeight: 900, margin: 0, color: '#fff', textTransform: 'uppercase', display: 'block' }}>
            {slide.highlight}
          </span>
        </motion.div>
      </motion.div>
        ...
        <motion.div variants={staggerContainer} className="grid-5-cols">
          {slide.items.map((item, idx) => (
            <motion.div key={idx} variants={itemAnim} className="card-glass" style={{ padding: 'clamp(24px, 3vw, 40px)', display: 'flex', flexDirection: 'column' }}>
              <span className="text-gradient-primary title-font card-num" style={{ fontWeight: 900, marginBottom: '12px', wordBreak: 'normal', minWidth: 'max-content' }}>{item.num}</span>
    ...
    return (
      <motion.div initial="hidden" whileInView="show" viewport={viewConfig}>
        <motion.div variants={staggerContainer} className="grid-3-cols">
          {slide.items.map((item, idx) => (
            <motion.div key={idx} variants={itemAnim} className="card-glass" style={{ padding: 'clamp(24px, 3vw, 40px)', display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 'clamp(10px, 1.5vw, 16px)', marginBottom: '14px' }}>
                <span className="text-gradient-primary title-font" style={{ fontWeight: 900, fontSize: 'clamp(1.8rem, 3.5vw, 2.8rem)', lineHeight: 1, flexShrink: 0, whiteSpace: 'nowrap', wordBreak: 'normal', minWidth: 'max-content' }}>{item.num}</span>
                <h3 className="title-font" style={{ fontWeight: 800, color: '#fff', margin: 0, textTransform: 'uppercase', lineHeight: 1.2, fontSize: 'clamp(0.85rem, 1.6vw, 1.25rem)' }}>{item.title}</h3>
              </div>
    ...
    return (
      <motion.div initial="hidden" whileInView="show" viewport={viewConfig} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
        ...
        <motion.div variants={itemAnim} className="pill-primary" style={{ marginBottom: 'clamp(50px, 8vw, 100px)' }}>
          <span className="title-font clamp-h2 text-3d" style={{ fontWeight: 900, margin: 0, color: '#fff', textTransform: 'uppercase', display: 'block' }}>{slide.highlight}</span>
        </motion.div>
        ...
        <motion.div variants={staggerContainer} className="card-glass" style={{ padding: 'clamp(50px, 6vw, 80px) clamp(24px, 4vw, 50px)', display: 'flex', flexWrap: 'wrap', gap: 'clamp(30px, 4vw, 60px)', position: 'relative', width: '100%', borderRadius: '32px' }}>
          ...
          {slide.items.map((item, idx) => (
            <motion.div key={idx} variants={itemAnim} style={{ flex: '1 1 200px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '16px' }}>
              <div className="title-font text-gradient-primary clamp-h1" style={{ fontWeight: 900, opacity: 0.3, whiteSpace: 'nowrap', wordBreak: 'normal', minWidth: 'max-content' }}>{item.num}</div>
  ...
  if (slide.type === 'payment-plan') {
    return (
      <motion.div initial="hidden" whileInView="show" viewport={viewConfig} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', textAlign: 'center' }}>
        ...
        <motion.div variants={itemAnim} className="pill-primary" style={{ marginBottom: 'clamp(50px, 8vw, 100px)' }}>
          <span className="title-font clamp-h2-large text-3d" style={{ fontWeight: 900, margin: 0, color: '#fff', textTransform: 'uppercase', display: 'block' }}>{slide.highlight}</span>
        </motion.div>
